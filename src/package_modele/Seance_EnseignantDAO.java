...
public class Seance_EnseignantDAO extends DAO<Seance_Enseignant> {
    ...
    public void delete(Seance_Enseignant obj) {
       ...
       try {
           st = connect.prepareStatement("DELETE FROM `seance_enseignants` WHERE `ID_Seance`=?");
           st.setInt(1,obj.getId_seance());
...
//                System.out.println(result.getInt("ID_Utilisateur"));
    ...
    public ArrayList<Integer> FindAllSeance(int id, int id2) {
        ...
        try {
            ResultSet result = this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM seance_enseignants WHERE ID_Utilisateur='" + id + "' and ID_Cours='" + id2 + "'");
            while (result.next()) {
                cours.add(result.getInt("ID_Seance"));
            }
        } catch (SQLException e) {
        }
        return cours;
